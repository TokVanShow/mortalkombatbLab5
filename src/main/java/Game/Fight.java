...
public class Fight {
    ...
    /**
     * Метод, вызывающийся при нажатии игроком кнопки "атаковать". В зависимости
     * от текущего хода, вызывает один из соответствующих ходу методов атаки.
     *
    ...
    /**
     * Метод, вызывающийся при нажатии игроком кнопки "защититься". В
     * зависимости от текущего хода, вызывает один из соответствующих ходу
     * методов защиты.
     *
     * @param game объект текущей игры
     */
    ...
    /**
     * Метод, вызывающийся при выборе кнопки "атаковать" в ход игрока. В
     * зависимости от действия противника (enemy.getAttack) определяет исход
     * атаки игрока.
     *
     * @param game объект текущей игры
     */
    public void playerAttackInHisTurn(Game game) {
        ...
        if (human.isStunned()) {
            ...
            enemy.setHealth(Math.max(enemy.getHealth() - human.getDamage(), 0));
            enemy.setStunned(false);
        } else {
            switch (enemy.getAttack()) {
                case 0 -> {
                    human.setHealth(Math.max(human.getHealth() - (enemy.getDamage() / 2), 0));
                ...
                case 2 -> {
                    ...
                    enemy.setHealth(Math.max(enemy.getHealth() - (human.getDamage() * 2), 0));
                }
            }
        }
    }
    ...
    /**
     * Метод, вызывающийся при выборе кнопки "атаковать" в ход противника. В
     * зависимости от действия противника (enemy.getAttack) определяет исход
     * атаки игрока.
     *
     * @param game объект текущей игры
    ...
    public void playerAttackInEnemyTurn(Game game) {
        ...
        if (human.isStunned()) {
            ...
            switch (enemy.getAttack()) {
                case 0 ->
                    game.setTurnResult(enemy.getName() + " didn't attack");
                case 1 -> {
                    human.setHealth(Math.max(human.getHealth() - enemy.getDamage(), 0));
    ...
    /**
     * Метод, вызывающийся при выборе кнопки "защититься" в ход игрока. В
     * зависимости от действия противника (enemy.getAttack) определяет исход
     * защиты игрока.
     *
     * @param game объект текущей игры
    ...
    public void playerDefenceInHisTurn(Game game) {
        ...
        if (human.isStunned()) {
            ...
            switch (enemy.getAttack()) {
                ...
                case 1 ->
                    game.setTurnResult("You didn't attack");
                case 2 -> {
                    game.setTurnResult(enemy.getName() + " are healing!");
                    enemy.setHealth(Math.min(enemy.getHealth() + (enemy.getMaxHealth() - enemy.getHealth()) / 2, enemy.getMaxHealth()));
                }
            }
        }
    }
    ...
    /**
     * Метод, вызывающийся при выборе кнопки "защититься" в ход противника. В
     * зависимости от действия противника (enemy.getAttack) определяет исход
     * защиты игрока.
     *
     * @param game объект текущей игры
    ...
    /**
     * Метод, вызывающийся при условии, что игрок в этот ход находится в
     * "стане". В зависимости от действий противника определяет исход хода.
     *
     * @param game объект текущей игры
    ...
    private void humanIsStunned(Game game, Player human, Player enemy) {
        switch (enemy.getAttack()) {
            case 0 ->
                game.setTurnResult("You are stunned, " + enemy.getName() + " didn't attack");
            case 1 -> {
    ...
    /**
     * Метод, вызываемый при окончании раунда, т.е. после выбранной атаки или
     * защиты игрока. Проверяет, не побежден ли один из участников.
     *
     * @param game объект текущей игры
     */
    public void endRoundCheck(Game game) {
        ...
        if (human.getHealth() <= 0 & game.getItem(2).getCount() > 0) {
            human.setHealth((int) (human.getMaxHealth() * 0.1));
    ...
    /**
     * Метод, вызываемый при победе игрока над противником. Случайным образом
     * выдает игроку предмет, опыт и очки в соответствии с уровнем противника.
     *
     * @param game объект текущей игры
     */
    ...
    /**
     * Метод, определяющий поведение противника на следующий ход врага и свой
     * ход. Поведение определяется только на 2 хода, после чего
     * переопределяется.
     *
     * @param game объект текущей игры
     */
     ...
     * Отталкивается от предыдущих действий игрока в ход игрока. Если игрок в
     * свой ход чаще выбирал защиту - противник будет атаковать, чтобы не
     * получать "стан". Если игрок чаще выбирал атаку - противник будет
     * защищаться для контратаки. При равных значениях противник выбирает
     * действие случайно. Для босса случайным образом (15%) может определиться
     * действие лечения.
     *
     * @param game объект текущей игры
     * @return действие противника: 0 - защита, 1 - атака, 2 (для босса) -
     * попытка лечения
     */
     ...
     * Отталкивается от предыдущих действий игрока в ход противника. Если игрок
     * в ход противника чаще выбирал защиту - противник будет защищаться, чтобы
     * иметь шанс "застанить" игрока. Если игрок чаще выбирал атаку - противник
     * будет атаковать, чтобы нанести урон. При равных значениях противник
     * выбирает действие случайно. Для босса случайным образом (15%) может
     * определиться действие лечения.
     *
     * @param game объект текущей игры
     * @return действие противника: 0 - защита, 1 - атака, 2 (для босса) -
     * попытка лечения
