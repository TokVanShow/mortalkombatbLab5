...
public class Game {
    ...
    private final Item[] items = {
            new Item("Малое зелье лечения", 0),
     ...
     * Создает игрока со стандартными характеристиками, противников для первой локации, задает первые два хода противника случайным образом.
     *
    ...
    public Game(int numberOfLocations) {
        ...
        this.human = new Human(0, 580, 18, 1, new ImageIcon(tk.getImage(iconUrl)));
        this.enemies = enemiesInThisLocation(human.getLevel());
        this.fight.enemyMoves.add((int) (Math.random() * 2));
        this.fight.enemyMoves.add((int) (Math.random() * 2));
        try {
            this.results = (ArrayList<Result>) ExcelReader.readFromExcel();
            assert results != null;
     ...
     * @return коллекция объектов Player, содержащая всех противников для этой локации
     */
     ...
     * Создан в классе Game для удобства модернизации приложения.
     *
    ...
    /**
     * Метод, вызываемый при нажатии кнопки "защищаться".
     * Создан в классе Game для удобства модернизации приложения.
     *
     * @param game объект текущей игры
     */
    ...
    /**
     * Метод, вызываемый при победе игрока над противником.
     * Случайным образом определяет выдаваемый предмет игроку (1 предмет - 60%, 2 предмет - 20%, 3 предмет - 10%).
     */
    public void AddItems() {
        double i = Math.random();
        if (i <= 0.60) {
            setItems(0, items[0].getCount() + 1);
        } else if ((i >= 0.61) & (i <= 0.80)) {
            setItems(1, items[1].getCount() + 1);
        } else if ((i >= 0.81) & (i <= 0.90)) {
            setItems(2, items[2].getCount() + 1);
        }
    }
            ...
            case 1:
                if (items[0].getCount() > 0) {
                    if (human.getHealth() + (human.getMaxHealth() * 0.25) >= human.getMaxHealth()) {
                        ...
                        human.setHealth((int) (human.getHealth() + (human.getMaxHealth() * 0.25)));
                    }
            ...
            case 2:
                if (items[1].getCount() > 0) {
                    if (human.getHealth() + (human.getMaxHealth() * 0.5) >= human.getMaxHealth()) {
                        ...
                        human.setHealth((int) (human.getHealth() + (human.getMaxHealth() * 0.5)));
                    }
